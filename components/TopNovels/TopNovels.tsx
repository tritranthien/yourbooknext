...
const TopNovels: React.FC<TopNovelsProps> = ({ novels, title }: TopNovelsProps) => {
    return <div className='w-full px-0'>
        <span className='text-primary-600 dark:text-primary-400 text-xl py-2 font-bold border-b-2 border-primary-200 dark:border-primary-900/50 w-full block'>{title}</span>
        <div className="flex flex-col md:flex-row w-full md:gap-1 lg:gap-3 py-2">
            {
                novels.length > 0 && <div className="flex flex-col md:w-1/3 pt-2">
                    ...
                    <span className="text-xl px-2 py-1 font-bold text-gray-800 dark:text-slate-100 hover:text-primary-600 dark:hover:text-primary-400 transition-colors block">
                    ...
                    <div className="flex w-full mt-2">
                        <span className="text-gray-500 dark:text-slate-400 text-xs px-2 mb-1 border-r border-gray-300 dark:border-slate-700 pr-2">
                            {novels[0].category?.cate || 'Chưa phân loại'}
                        </span>
                        <span className="text-secondary-600 dark:text-secondary-400 text-xs px-2 mb-1">{`${novels[0].chapCount} chương`}</span>
                    </div>
                    {novels[0].author?.slug ? (
                        <Link legacyBehavior passHref href={`/tac-gia/${novels[0].author.slug}`}>
                            <a className="text-secondary-500 hover:text-secondary-700 dark:hover:text-secondary-400 px-2 mb-1 block text-sm font-medium">{novels[0].author.name}</a>
                        </Link>
                    ) : (
                        <span className="text-secondary-500 px-2 mb-1 block text-sm font-medium">{novels[0].author?.name || 'Đang cập nhật'}</span>
                    )}
                    <p className="text-sm text-gray-400 dark:text-slate-500 line-clamp-3 px-2 mb-4 leading-relaxed">{novels[0].description}</p>
                </div>
            }
            <div className="flex md:w-2/3 md:items-start flex-wrap content-start">
                {
                    novels.map((item, index) => {
                        ...
                        return <div className='md:w-1/2 lg:w-1/3 w-full p-2 flex items-center border-b md:border-none border-gray-100 dark:border-slate-800/50 py-3' key={index}>
                            <div className="flex w-full items-start">
                                ...
                                <div className="flex-1 w-0 mx-3 flex flex-col justify-between h-[110px] py-1">
                                    ...
                                    <span className="w-full text-sm font-bold text-gray-800 dark:text-slate-200 leading-tight mb-1 hover:text-primary-600 dark:hover:text-primary-400 transition-colors block">
                                    ...
                                    <span className="w-full text-xs text-secondary-500 dark:text-secondary-400 font-medium mb-1 truncate">
                                        {item.author?.slug ? (
                                    ...
                                    <div className="mt-auto flex flex-col gap-1">
                                        ...
                                        <span className="text-primary-500 dark:text-primary-400 text-xs font-semibold">
