                     ...
                     
                        {
                          isSuccess && data?.map((item,index)=>{
                            return <li key={index} className="flex py-1">
                              <span className="px-2 font-bold md:w-auto w-1/2 line-clamp-1"><Link passHref href={`/truyen/${item.novel.slug}`}><a className="text-sky-500">{item.novel.title}</a></Link></span>
