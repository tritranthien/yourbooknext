    ...
    <ul className="w-full list-none">
        {
            Coldata.map((item,index)=>{
                return <li key={index} className="flex py-1 w-full">
                    ...
                    <span className="w-[40px] text-xs truncate text-gray-400"><Link legacyBehavior passHref href={item.chapCount > 0 ? `/truyen/${item.slug}/${item.chapCount}` : `/truyen/${item.slug}`}><a>{`C.${item.chapCount}`}</a></Link></span>
                </li>
            })
        }
    </ul>
